...
@section('content')
    <main id="main" class="main">
        <div class="data-laporan">
            Detail Laba dan Rugi - Tahun {{ $year }}
        </div><!-- End Page Title -->
                                    ...
                                    <tbody>
                                        @foreach ($report as $month => $data)
                                            <tr>
                                                ...
                                                <td>Rp {{ number_format($data['gaji']['total_upah'], 0, ',', '.') }}</td>
                                                <td>Rp {{ number_format($data['iuran'], 0, ',', '.') }}</td>
                                                <td>Rp {{ number_format($data['cost'], 0, ',', '.') }}</td>
                                                <td>Rp
                                                    {{ number_format($data['sampahkeluar']['total_sampahkeluar'], 0, ',', '.') }}
                                                </td>
                                                <td>Rp
                                                    {{ number_format($data['iuran'] + $data['sampahkeluar']['total_sampahkeluar'] - ($data['gaji']['total_upah'] + $data['cost']), 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
